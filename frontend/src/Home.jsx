  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      {bursts.map((b) => (
        <div 
          key={b.id} 
          className="absolute w-2 h-2 rounded-full animate-firework opacity-0"
          style={{ 
            left: `${b.left}%`, 
            top: `${b.top}%`, 
            backgroundColor: b.color,
            boxShadow: `0 0 20px 5px ${b.color}`,
...
export default function Home({ user, onLoginSuccess, onLogout }) {
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden transition-colors duration-300 bg-[#050505] text-zinc-100">
      ...
      <nav className="fixed top-0 w-full z-50 flex justify-center pt-8 px-4">
        <div className="flex items-center justify-between w-full max-w-5xl px-5 py-3 rounded-full border shadow-xl backdrop-blur-xl transition-all duration-300 bg-zinc-900/60 border-white/10">
          ...
          <button onClick={() => navigate('/')} className="flex items-center gap-3 hover:opacity-80 transition group">
            <div className="w-9 h-9 bg-gradient-to-tr from-blue-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg group-hover:rotate-12 transition-transform">
      ...
      <main className="relative z-10 flex flex-col items-center pt-40 px-4 pb-20 max-w-7xl mx-auto">
            ...
            <div className="relative p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                ...
                <div className="flex-1">
                    <h3 className="text-3xl font-bold flex flex-wrap items-center justify-center sm:justify-start gap-3 text-white">
                        Happy New Year <Snowflake size={24} className="text-cyan-400 animate-spin-slow" />
                        <span className="text-sm font-normal px-2 py-1 bg-gradient-to-r from-yellow-500 to-amber-600 text-black font-bold rounded-md">2026</span>
                    </h3>
                    <p className="mt-3 text-lg leading-relaxed font-medium text-zinc-300">Start your year with powerful AI creativity and intelligence.</p>
                </div>
            </div>
        </div>
        ...
        <div className="w-full max-w-2xl mx-auto relative group z-20 mb-20">
            <div className="absolute -inset-1 bg-gradient-to-r from-yellow-500 via-purple-500 to-cyan-500 rounded-full blur opacity-30 group-hover:opacity-70 transition duration-500 animate-pulse"></div>
            ...
            >
                ...
                <div className="flex-1 text-xl font-medium opacity-60">Ask Cneapee to plan your year...</div>
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-2">
            {FEATURES.map((item, idx) => (
              <div 
                key={idx} 
                onClick={() => handleProtectedNav(`/${item.id}`)}
